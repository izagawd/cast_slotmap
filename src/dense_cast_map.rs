//! Safe wrapper around [`UnsafeDenseCastMap`](crate::unsafe_dense_cast_map::UnsafeDenseCastMap)
//! that binds keys to the map that created them via a [`MapId`].
//!
//! This is the dense counterpart of [`CastMap`](crate::cast_map::CastMap): the
//! same API and identity guarantees, but backed by [`slotmap::DenseSlotMap`] for
//! fast contiguous iteration (see [`UnsafeDenseCastMap`] for the storage
//! trade-offs).
//!
...
use crate::unsafe_dense_cast_map::{self, UnsafeDenseCastMap};
...
/// A safe wrapper around [`UnsafeDenseCastMap`] that checks a per-map [`MapId`]
/// on every keyed access.
///
/// `Ptr` is the stored smart pointer (e.g. `Box<dyn Any>`). Use the alias
/// [`BoxDenseCastMap`] for the common `Box` case.
pub struct DenseCastMap<K: Key, Ptr>
where
    Ptr: StableDeref,
{
    inner: UnsafeDenseCastMap<K, Ptr>,
    map_id: MapId,
}

// ─── Clone ───────────────────────────────────────────────────────────────────

impl<K: Key, Ptr> Clone for DenseCastMap<K, Ptr>
where
    Ptr: StableDeref,
    UnsafeDenseCastMap<K, Ptr>: Clone,
{
...
impl<K: Key, Ptr> Default for DenseCastMap<K, Ptr>
where
    Ptr: StableDeref,
{
    fn default() -> Self {
...
impl<K: Key, Ptr> DenseCastMap<K, Ptr>
where
    Ptr: StableDeref,
{
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            inner: UnsafeDenseCastMap::with_capacity(capacity),
            map_id: MapId::next(),
        }
    }
    ...
    /// Consumes this map and returns the underlying [`UnsafeDenseCastMap`].
    #[inline]
    pub fn inner(self) -> UnsafeDenseCastMap<K, Ptr> {
        self.inner
    }

    /// Returns a shared reference to the underlying [`UnsafeDenseCastMap`].
    #[inline]
    pub fn inner_ref(&self) -> &UnsafeDenseCastMap<K, Ptr> {
        &self.inner
    }

    /// Returns a mutable reference to the underlying [`UnsafeDenseCastMap`].
    #[inline]
    pub fn inner_mut(&mut self) -> &mut UnsafeDenseCastMap<K, Ptr> {
        &mut self.inner
    }
    ...
    /// but keeping the slot reservable (dense-only). Pairs with
    ...
    pub fn reattach_sized<ConcretePtr>(
        &mut self,
        key: StableCastKey<ConcretePtr::Target, K>,
        value: ConcretePtr,
    ) -> bool
    where
    ...
    <Ptr::Target as Pointee>::Metadata: Copy,
{
    ...
    /// different map. Pairs with the `reattach` methods. Dense-only.
    #[inline]
...
impl<K: Key, Ptr> std::ops::Index<StableCastKey<Ptr::Target, K>> for DenseCastMap<K, Ptr>
where
    Ptr: StableDeref,
    Ptr::Target: Pointee,
    <Ptr::Target as Pointee>::Metadata: Copy,
...
impl<K: Key, Ptr> std::ops::IndexMut<StableCastKey<Ptr::Target, K>> for DenseCastMap<K, Ptr>
where
    Ptr: StableDeref + DerefMut,
    Ptr::Target: Pointee,
    <Ptr::Target as Pointee>::Metadata: Copy,
...
where
    ...
    Ptr: 'a,
{
    inner: unsafe_dense_cast_map::Iter<'a, K, Ptr>,
    map_id: MapId,
}
...
where
    ...
    Ptr: 'a,
{
    inner: unsafe_dense_cast_map::IterMut<'a, K, Ptr>,
    map_id: MapId,
}
...
where
    ...
    Ptr: 'a,
{
    inner: unsafe_dense_cast_map::Drain<'a, K, Ptr>,
    map_id: MapId,
}
...
pub struct IntoIter<K: Key, Ptr: StableDeref> {
    inner: unsafe_dense_cast_map::IntoIter<K, Ptr>,
    map_id: MapId,
}
...
impl<K: Key, Ptr> IntoIterator for DenseCastMap<K, Ptr>
where
    Ptr: StableDeref,
    Ptr::Target: Pointee,
    <Ptr::Target as Pointee>::Metadata: Copy,
...
impl<'a, K: Key, Ptr> IntoIterator for &'a DenseCastMap<K, Ptr>
where
    K: 'a,
    Ptr: StableDeref + 'a,
    Ptr::Target: Pointee,
...
impl<'a, K: Key, Ptr> IntoIterator for &'a mut DenseCastMap<K, Ptr>
where
    K: 'a,
    Ptr: StableDeref + DerefMut + 'a,
    Ptr::Target: Pointee,
